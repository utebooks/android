...
class PathInterpolatorGingerbread implements Interpolator {
    ...
    public PathInterpolatorGingerbread(Path path) {
        ...
        this.f23mX = new float[numPoints];
        this.f24mY = new float[numPoints];
        float[] position = new float[2];
        for (int i = 0; i < numPoints; i++) {
            pathMeasure.getPosTan((((float) i) * pathLength) / ((float) (numPoints - 1)), position, (float[]) null);
            this.f23mX[i] = position[0];
            this.f24mY[i] = position[1];
        }
    }
    ...
    public float getInterpolation(float t) {
        ...
        int startIndex = 0;
        int endIndex = this.f23mX.length - 1;
        while (endIndex - startIndex > 1) {
            int midIndex = (startIndex + endIndex) / 2;
            if (t < this.f23mX[midIndex]) {
                endIndex = midIndex;
            } else {
                startIndex = midIndex;
            }
        }
        float xRange = this.f23mX[endIndex] - this.f23mX[startIndex];
        if (xRange == 0.0f) {
            return this.f24mY[startIndex];
        }
        float startY = this.f24mY[startIndex];
        return ((this.f24mY[endIndex] - startY) * ((t - this.f23mX[startIndex]) / xRange)) + startY;
    }
