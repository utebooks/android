...
public class DiffUtil {
    ...
    public static DiffResult calculateDiff(Callback cb, boolean detectMoves) {
        ...
        while (!arrayList.isEmpty()) {
            ...
            if (snake != null) {
                ...
                Range right = range;
                if (!snake.reverse) {
                    right.oldListStart = snake.f33x + snake.size;
                    right.newListStart = snake.f34y + snake.size;
                } else if (snake.removal) {
                    right.oldListStart = snake.f33x + snake.size + 1;
                    right.newListStart = snake.f34y + snake.size;
                } else {
                    right.oldListStart = snake.f33x + snake.size;
                    right.newListStart = snake.f34y + snake.size + 1;
                }
                arrayList.add(right);
            } else {
    ...
    public static class DiffResult {
        ...
        public void dispatchUpdatesTo(final RecyclerView.C0337Adapter adapter) {
        ...
        public void dispatchUpdatesTo(ListUpdateCallback updateCallback) {
            ...
            for (int snakeIndex = this.mSnakes.size() - 1; snakeIndex >= 0; snakeIndex--) {
                ...
                int endX = snake.f33x + snakeSize;
                int endY = snake.f34y + snakeSize;
                if (endX < posOld) {
