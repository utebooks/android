...
public final class GooglePlayServicesUtil extends zze {
    ...
    private static void zza(int i, Context context, String str, PendingIntent pendingIntent) {
        ...
        if (zzg == null) {
            zzg = resources.getString(C0380R.string.common_google_play_services_notification_ticker);
        }
        String zzc = zzh.zzc(context, i, zzam);
        if (com.google.android.gms.common.util.zzi.zzaB(context)) {
            zzaa.zzai(zzs.zzvb());
            build = new Notification.Builder(context).setSmallIcon(C0380R.C0381drawable.common_ic_googleplayservices).setPriority(2).setAutoCancel(true).setStyle(new Notification.BigTextStyle().bigText(new StringBuilder(String.valueOf(zzg).length() + 1 + String.valueOf(zzc).length()).append(zzg).append(" ").append(zzc).toString())).addAction(C0380R.C0381drawable.common_full_open_on_phone, resources.getString(C0380R.string.common_open_on_phone), pendingIntent).build();
        } else {
            String string = resources.getString(C0380R.string.common_google_play_services_notification_ticker);
            if (zzs.zzuX()) {
