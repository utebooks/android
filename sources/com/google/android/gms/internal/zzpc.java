...
public final class zzpc extends Fragment implements zzor {
    ...
    public static zzpc zza(FragmentActivity fragmentActivity) {
        ...
        if (weakReference == null || (zzpc = (zzpc) weakReference.get()) == null) {
            try {
                zzpc = (zzpc) fragmentActivity.getSupportFragmentManager().findFragmentByTag("SupportLifecycleFragmentImpl");
                if (zzpc == null || zzpc.isRemoving()) {
                    zzpc = new zzpc();
                    fragmentActivity.getSupportFragmentManager().beginTransaction().add((Fragment) zzpc, "SupportLifecycleFragmentImpl").commitAllowingStateLoss();
                }
                zzaoq.put(fragmentActivity, new WeakReference(zzpc));
