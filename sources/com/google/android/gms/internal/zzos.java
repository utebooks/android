...
public final class zzos extends Fragment implements zzor {
    ...
    public static zzos zzt(Activity activity) {
        ...
        if (weakReference == null || (zzos = (zzos) weakReference.get()) == null) {
            try {
                zzos = (zzos) activity.getFragmentManager().findFragmentByTag("LifecycleFragmentImpl");
                if (zzos == null || zzos.isRemoving()) {
                    zzos = new zzos();
                    activity.getFragmentManager().beginTransaction().add(zzos, "LifecycleFragmentImpl").commitAllowingStateLoss();
                }
                zzaoq.put(activity, new WeakReference(zzos));
